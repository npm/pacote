...
class RegistryFetcher extends Fetcher {
  ...
  async manifest () {
    ...
    const mani = await pickManifest(packument, this.spec.fetchSpec, {
      ...
      // TODO: https://github.com/npm/package-json/pull/96
      // new PackageJson().fromContent(pkg).normalize()
      .then((pkg) => new PackageJson().fromJSON(JSON.stringify(pkg)).normalize())
      .then(pkg => pkg.content)
