...
class DirFetcher extends Fetcher {
  ...
  #prepareDir () {
    return this.manifest().then(async mani => {
      if (!mani.scripts || !(mani.scripts.prepare || (process.env._PACOTE_FROM_GIT_ === 'yes' && mani.scripts.prepack))) {
        return
      }
