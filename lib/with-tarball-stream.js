    ...
    return statAsync(file)
      ...
      .catch((err) => {
        if (err.code === 'EINTEGRITY') {
          opts.log.warn(
            'pacote',
            `EINTEGRITY while extracting ${spec} from ${file}.You will have to recreate the file.`
          )
    ...
    if (
      ...
      return streamHandler(stream).catch((err) => {
        if (err.code === 'EINTEGRITY' || err.code === 'Z_DATA_ERROR') {
          opts.log.warn(
            'tarball',
            `cached data for ${spec} (${opts.integrity}) seems to be corrupted. Refreshing cache.`
          )
          ...
          (tryAgain, attemptNum) => {
            ...
            return new Promise((resolve, reject) => {
              ...
              if (
                opts.cache &&
                err.code &&
                !String(err.code).match(/^E\d{3}$/)
              ) {
                if (err.code === 'EINTEGRITY' || err.code === 'Z_DATA_ERROR') {
                  opts.log.warn(
                    'tarball',
                    `tarball data for ${spec} (${opts.integrity}) seems to be corrupted. Trying one more time.`
                  )
                }
                return cleanUpCached(opts.cache, err.sri, opts).then(() =>
                  tryAgain(err)
                )
              } else {
                throw err
