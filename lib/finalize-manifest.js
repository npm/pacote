...
function finalizeManifest (pkg, spec, opts) {
  ...
  return cachedManifest.then((cached) => {
    if (cached && cached.metadata && cached.metadata.manifest) {
      ...
      return tarballedProps(pkg, spec, opts)
        .then((props) => {
          return pkg && pkg.name
...
function tarballedProps (pkg, spec, opts) {
  const needsShrinkwrap =
    !pkg || (pkg._hasShrinkwrap !== false && !pkg._shrinkwrap)
  const needsBin = !!(
    !pkg ||
    (!pkg.bin && pkg.directories && pkg.directories.bin)
  )
  ...
  if (!needsShrinkwrap && !needsBin && !needsHash && !needsManifest) {
    ...
    return Promise.all([
      ...
      needsHash &&
        ssri.fromStream(tarStream, { algorithms: ['sha1', 'sha512'] }),
      needsExtract &&
        pipe(
          tarStream,
          extracted
        )
    ]).then(([sr, mani, paths, hash]) => {
      if (needsManifest && !mani) {
        const err = new Error(
          `Non-registry package missing package.json: ${spec}.`
        )
          ...
          paths.forEach((filePath) => {
            if (minimatch(filePath, dirBin + '/**')) {
              ...
              if (relative && relative[0] !== '.') {
                extraProps.bin[path.basename(relative)] = path.join(
                  dirBin,
                  relative
                )
              }
      ...
      return Object.assign(extraProps, {
        _shrinkwrap: sr,
        _resolved:
          (mani && mani._resolved) || (pkg && pkg._resolved) || spec.fetchSpec,
        _integrity:
          needsIntegrity && hash && hash.sha512 && hash.sha512[0].toString(),
...
function getPaths (dataStream) {
  return new Promise((resolve, reject) => {
    let paths = []
    dataStream.on('error', reject)
    dataStream.on('close', () => resolve(paths))
