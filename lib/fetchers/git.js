...
Fetcher.impl(fetchGit, {
  ...
  tarball (spec, opts) {
    ...
    this.manifest(spec, opts)
      .then((manifest) => {
        ...
        return pipe(
          this.fromManifest(manifest, spec, opts).on('integrity', (i) =>
  ...
  fromManifest (manifest, spec, opts) {
    ...
    const cacheStream =
      opts.cache &&
      cacache.get
        .stream(opts.cache, cacheKey('packed-dir', cacheName), opts)
...
function plainManifest (repo, spec, opts) {
  ...
  return resolve(repo, spec, spec.name, opts).then((ref) => {
    if (ref) {
      ...
      const resolved = spec.saveSpec.replace(
        /(?:#.*)?$/,
        rawRef ? `#${rawRef}` : ''
      )
      return {
...
function resolve (url, spec, name, opts) {
  ...
  return git.revs(url, opts).then((remoteRefs) => {
    return isSemver
      ...
      : remoteRefs
        ? Promise.resolve(
          remoteRefs.refs[spec.gitCommittish] ||
            remoteRefs.refs[remoteRefs.shas[spec.gitCommittish]]
        )
        : null
  })
}
...
function withTmp (opts, cb) {
  if (opts.cache) {
    ...
    return mkdirp(tmpName).then((tmpName) =>
      cb(tmpName)
        .then((result) => disposer(tmpName, result))
        .catch((err) => disposer(tmpName, err, true))
    )
  }
}
