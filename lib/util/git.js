...
function shouldRetry (error, number) {
  return (
    GIT_TRANSIENT_ERROR_RE.test(error) &&
    ...
    return promiseRetry(
      (retry, number) => {
        if (number !== 1) {
          opts.log.silly(
            'pacote',
            'Retrying git command: ' +
              gitArgs.join(' ') +
              ' attempt # ' +
              number
          )
        }
        return execFileAsync(gitPath, gitArgs, mkOpts(gitOpts, opts)).catch(
          (err) => {
  ...
  return checkGit(opts).then((gitPath) => {
    return promiseRetry((retry, number) => {
      if (number !== 1) {
        opts.log.silly(
          'pacote',
          'Retrying git command: ' + gitArgs.join(' ') + ' attempt # ' + number
        )
      }
...
function checkGit (opts) {
  if (opts.git) {
    return Promise.resolve(opts.git)
  } else if (!GITPATH) {
