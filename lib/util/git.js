...
function headSha (repo, opts) {
  return execGit(['rev-parse', '--revs-only', 'HEAD'], { cwd: repo }, opts)
  ...
  new Promise((resolve, reject) => {
    ...
    child.on('close', (code, signal) => {
      if (code === 0 && signal === null)
        ...
        return reject(new Error('exited with error code: ' + code))
      else
        return reject(new Error('killed by signal: ' + signal))
    })
  })
...
const checkGit = opts =>
  opts.git ? Promise.resolve(opts.git)
    : GITPATH ? Promise.resolve(GITPATH)
    : Promise.reject(Object.assign(
      new Error('No git binary found in $PATH'), {
        code: 'ENOGIT'
      }))
...
const HEAD = 'HEAD'
const refType = ref =>
  ref.indexOf(REFS_TAGS) !== -1 ? 'tag'
    : ref.indexOf(REFS_HEADS) !== -1 ? 'branch'
