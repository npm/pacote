...
const maybeShallow = (repo, opts) =>
  opts.gitShallow || opts.gitShallow === false ? opts.gitShallow
    : shallowHosts.has(parse(repo).host)

const isWindows = opts =>
  process.platform === 'win32' || opts.fakePlatform === 'win32'
...
const other = (repo, revDoc, target, opts) => {
  ...
  const checkoutSha = [ 'checkout', revDoc.sha, '--recurse-submodules' ]
    .concat(shallow ? ['--shallow-submodules'] : [])
...
const unresolved = (repo, ref, target, opts) => {
  ...
  // but we can avoid checking out the working dir twice, and check out
  // shallow submodules if supported.
  const lp = isWindows(opts) ? ['--config', 'core.longpaths=true'] : []
  const cloneArgs = ['clone', '--mirror', '-q', repo, target + '/.git']
  const git = (args) => spawn(args, { cwd: target }, opts)
  const shallow = maybeShallow(repo, opts)
  const checkout = ['checkout', ref, '--recurse-submodules']
    .concat(shallow ? ['--shallow-submodules'] : [])

  return mkdirp(target)
    .then(() => git(cloneArgs.concat(lp)))
    .then(() => git(['init']))
    .then(() => git(['checkout', ref, '--recurse-submodules']))
