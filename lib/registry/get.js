  ...
  tryFromCache(key, opts, function (err, cachedBody, meta) {
    ...
    if (cachedBody && !meta) {
      ...
      client(opts).get(uri, {
        ...
        if (err && cachedBody) {
          opts.log.silly('get', 'request error. Using', key, 'from cache.')
          return cb(null, cachedBody)
        } else if (err) {
          return cb(err)
        } else {
          ...
          opts.log.silly('get', 'inserting', key, 'into cache.')
...
function tryFromCache (key, opts, cb) {
  if (MEMOIZED[key]) {
    opts.log.silly('get', key, 'already memoized.')
    return cb(null, MEMOIZED[key])
  } else if (!opts.cache) {
    opts.log.silly('get', key, 'not memoized, no cache available.')
    return cb(null)
  } else {
    cacache.get.info(opts.cache, key, function (err, info) {
      if (err) { return cb(err) }
      if (!info || !info.path) {
        opts.log.silly('get', key, 'not found in cache')
      ...
      fs.readFile(filePath, 'utf8', function (err, json) {
        ...
        opts.log.silly('get', key, 'fetched from cache.')
