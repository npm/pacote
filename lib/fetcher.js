...
class FetcherBase {
  constructor (spec, opts) {
    ...
    this.fullMetadata = opts.enjoyBy ? true
  ...
  get cache () {
    if (this.opts.cache)
      return this.opts.cache
    throw new Error('a cache option is required for this operation')
  }
  ...
  tarballStream (streamHandler) {
    ...
    const fromResolved = er => {
      ...
      return this.resolve().then(() => retry(tryAgain => {
        const sh = streamHandler(this[_istream](this[_tarballFromResolved]()))
        return !this.hasCache ? sh : sh.catch(er => {
  ...
  cleanupCached () {
    return this.hasCache
      ? cacache.rm.content(this.cache, this.integrity, this.opts)
      : Promise.resolve(null)
  }

  [_chown] (path, uid, gid) {
    return selfOwner && (selfOwner.uid !== uid || selfOwner.gid !== gid)
      ? chownr(path, uid, gid)
      : null
  }
  ...
  [_mkdir] (dest) {
    ...
    return !inferOwner
      ...
      : inferOwner(dest).then(({uid, gid}) =>
        rimraf(dest)
        .then(() => mkdirp(dest))
        .then(made => this[_chown](made || dest, uid, gid))
        .then(() => ({uid, gid})))
  }
  ...
  [_tarxOptions] ({ cwd, uid, gid }) {
    ...
    return {
      ...
      strip: 1,
      onwarn: this.log ? msg => this.log.warn('tar', msg) : null,
      uid,
      gid,
      umask: this.umask,
    }
