...
test('automatically handles gzipped tarballs', (t) => {
  ...
  return mockTar(pkg, { gzip: true, stream: true })
    .then((tarStream) => {
      return pipe(
        tarStream,
        extractStream('foo@1', './', OPTS)
      )
    })
    .then(() => {
      return Promise.all([
...
test('strips first item in path, even if not `package/`', (t) => {
  ...
  return mockTar(pkg, { noPrefix: true, stream: true })
    .then((tarStream) => {
      return pipe(
        tarStream,
        extractStream('foo@1', './', OPTS)
    ...
    .then(() => {
      return Promise.all([
        ...
        t.deepEqual(json, pkg['package/package.json'], 'flattened package.json')
        t.equal(indexjs, pkg['something-else/index.js'], 'flattened index.js')
      })
    })
})
  ...
  return mockTar(pkg, { stream: true })
    ...
    .then(() => {
      return Promise.all([
        ...
        stat('./linky').then(
          (s) => {
            throw new Error('this was supposed to error' + JSON.stringify(s))
          },
          (err) => {
    ...
    .then(() => {
      return mockTar(
        ...
        .then((tarStream) => {
          return pipe(
            tarStream,
            extractStream('foo@1', './no-npmignore', OPTS)
          )
        })
        .then(() => {
          return readFile('./no-npmignore/.npmignore', 'utf8').then((data) => {
    ...
    .then(() => {
      return mockTar(
        ...
        .then((tarStream) => {
          return pipe(
            tarStream,
            extractStream('foo@1', './has-npmignore1', OPTS)
          )
        })
        .then(() => {
          return Promise.all([
            readFile('./has-npmignore1/.npmignore', 'utf8').then((data) => {
              t.deepEqual(data, 'npm!', '.npmignore left intact if present')
            }),
            readFile('./has-npmignore1/.gitignore', 'utf8').then(
              () => {
    ...
    .then(() => {
      return mockTar(
        ...
        .then((tarStream) => {
          return pipe(
            tarStream,
            extractStream('foo@1', './has-npmignore2', OPTS)
          )
        })
        .then(() => {
          return Promise.all([
            readFile('./has-npmignore2/.npmignore', 'utf8').then((data) => {
              t.deepEqual(data, 'npm!', '.npmignore left intact if present')
            }),
            readFile('./has-npmignore2/.gitignore', 'utf8').then((data) => {
              t.deepEqual(
                data,
                'git!',
                '.gitignore intact if we previously had an .npmignore'
              )
            })
          ])
        })
...
test(
  'accepts dmode/fmode/umask opts',
  {
    ...
    return mockTar(pkg, { stream: true })
      ...
      .then(() => {
        return Promise.all([
          ...
          stat('./foo').then((s) => {
            t.equal(s.mode & 0o777, 0o755, 'mode set as expected')
          }),
          stat('./foo/index.js').then((s) => {
            t.equal(s.mode & 0o777, 0o644, 'fmode set as expected')
          }),
          stat('./bin/cli.js').then((s) => {
            t.equal(s.mode & 0o777, 0o755, 'preserved execute bit as expected')
          })
        ])
      })
  }
