...
test('supports directory deps', (t) => {
  ...
  return mkdirp(path.join(PKG, 'x'))
    .then(() => {
      return Promise.all([
        writeFile(path.join(PKG, 'package.json'), JSON.stringify(pkg)),
        writeFile(path.join(PKG, 'npm-shrinkwrap.json'), JSON.stringify(sr)),
        writeFile(path.join(PKG, 'x', 'mybin'), 'console.log("hi there")')
      ])
    })
    .then(() => {
      return manifest(PKG)
    ...
    .then(() => {
      return Promise.all([
        readFile(path.join(EXT, 'package.json'), 'utf8'),
        readFile(path.join(EXT, 'npm-shrinkwrap.json'), 'utf8'),
        readFile(path.join(EXT, 'x', 'mybin'), 'utf8')
      ]).then(([xpkg, xsr, xbin]) => {
